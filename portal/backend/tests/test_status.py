...
async def test_platform_status(client) -> None:
    ...
    with (
        ...
        patch("app.api.status._azure_compute_cluster_status", new_callable=AsyncMock, return_value={"status": "unknown"}),
        patch("app.api.status._compute_workload_probes", new_callable=AsyncMock, return_value=[]),
    ):
