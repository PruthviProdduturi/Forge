...
helm upgrade --install hive-metastore \
  ...
  --set db.password="" \
  --set serviceAccount.annotations."azure\.workload\.identity/client-id"=$(az identity show -g rg-forge-{env} -n id-forge-spark --query clientId -o tsv) \
  --wait --timeout 5m
```
...
helm upgrade --install trino \
  ...
  --set image.tag=438 \
  --set serviceAccount.annotations."azure\.workload\.identity/client-id"=$(az identity show -g rg-forge-{env} -n id-forge-trino --query clientId -o tsv) \
  --wait --timeout 10m
```
