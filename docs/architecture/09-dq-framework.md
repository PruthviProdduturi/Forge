...
The DQ framework provides two integration points depending on how the pipeline is authored:

| Pattern | When to use |
|---------|-------------|
| `ForgeDqGateOperator` | All pipelines generated by `forge generate`. The operator submits the `forge_dq_gate` platform Spark job as a separate Airflow task after the ingest/transform task. This is the standard pattern. |
| `@track` decorator | Inline DQ within a Python Spark job. DQ runs in the same Spark session immediately after the DataFrame write. Used for hand-authored jobs where the full DQ gate approach is not appropriate. |

### ForgeDqGateOperator — Standard Pattern

`ForgeDqGateOperator` (from the `forge_airflow` plugin) is the DQ gate for all generated pipelines. It submits `forge_dq_gate.py` as a `SparkApplication` to the compute cluster. The gate job:

1. Reads `RULES_PATH` from its environment (`abfss://code@{storage}/dq/rules/{job}.yaml`)
2. Downloads the YAML from ADLS via the Hadoop FS API to a temp file
3. Instantiates `DQRunner` and evaluates all three DQ layers
4. Applies partition-aware filtering: bronze jobs filter by `__year/__month/__day`; silver and gold jobs filter by `__date = DD_MM_YYYY_HH`
5. Fails the Airflow task on any critical rule violation, blocking all downstream tasks
...
The gate job lives at `orchestration/spark-jobs/forge_dq_gate.py` and is deployed to `code/spark/jobs/forge_dq_gate.py` in ADLS by `sync-jobs.sh`. It is a platform job — data engineers do not modify it.

### forge_dq_gate.py — Platform DQ Spark Job

`forge_dq_gate.py` is the generic DQ gate job shared by all pipelines. It is not generated per-pipeline. Key behaviour:

- Reads `RULES_PATH` env var — the full `abfss://` path to the rules YAML for this pipeline and layer
- Downloads the YAML from ADLS using the Hadoop FS API (no additional credentials — workload identity)
- Instantiates `DQRunner` against the target table
- **Partition filtering by layer:**
  - Bronze: filters using the four integer partition columns `__year`, `__month`, `__day`, `__hour` passed as env vars
  - Silver/Gold: filters using the single string column `__date = DD_MM_YYYY_HH`
- Exits non-zero on any critical failure → Airflow task status `FAILED` → downstream tasks blocked
...
The decorator reads `FORGE_DQ_ENABLED` and `FORGE_DQ_FAIL_ON_CRITICAL` from the environment. If `FORGE_DQ_ENABLED=false`, the decorator is a no-op.

### DAG Example: Bronze → Silver with DQ Gates (Generated)
...
**Failure propagation:** If `dq_gate_silver` fails (a critical rule fails), any downstream tasks are blocked with status `upstream_failed`. The Gold layer is never written with bad Silver input.
