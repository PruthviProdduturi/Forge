...
|-------------------|---------|-------------|
| `id-forge-spark-{alias}-{env}` | Spark Operator pods (compute cluster) | Storage Blob Data Contributor (bronze/silver/gold/code) · KV Secrets User |
| `id-forge-trino-{alias}-{env}` | Trino query pods + auth proxy (compute cluster) | Storage Blob Data Reader (bronze/silver/gold) · KV Secrets User |
