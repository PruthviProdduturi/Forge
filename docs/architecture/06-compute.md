...
4. [Spark SDK Distribution](#4-spark-sdk-distribution)
5. [Spark Configuration Hierarchy](#5-spark-configuration-hierarchy)
6. [Node Pool Assignment](#6-node-pool-assignment)
7. [Trino Cluster](#7-trino-cluster)
8. [Trino ADLS Access](#8-trino-adls-access)
9. [Trino Authentication](#9-trino-authentication)
10. [Trino Autoscaling](#10-trino-autoscaling)
11. [Hive Metastore](#11-hive-metastore)
12. [Compute Cost Tracking](#12-compute-cost-tracking)
13. [Full Compute Cluster Topology](#13-full-compute-cluster-topology)
...
- `Storage Blob Data Contributor` on `code/` container (covers job scripts and `checkpoints/<pipeline_id>/` paths)
...
| Code/Jobs | `abfss://code@<account>.dfs.core.windows.net/spark/jobs/` | Per-job Python entry points |
| Code/Lib | `abfss://code@<account>.dfs.core.windows.net/lib/` | Reserved — SDK is baked into Spark image |
| Code/DQ | `abfss://code@<account>.dfs.core.windows.net/dq/rules/` | DQ YAML rules |
| Deploy State | `abfss://code@<account>.dfs.core.windows.net/state/` | Deployment history (sync-jobs.sh) |
| Checkpoints | `abfss://code@<account>.dfs.core.windows.net/checkpoints/<pipeline_id>/` | Structured Streaming checkpoints |
| DQ Results | `abfss://silver@<account>.dfs.core.windows.net/_platform/dq_results/` | Platform internal |

---

## 4. Spark SDK Distribution

### Approach

Every Spark job imports `forge_sdk` and `forge_dq`. Both are **proper pip packages** (with `pyproject.toml`) baked directly into the Spark Docker image at build time:

```dockerfile
# infra/docker/spark/Dockerfile
COPY sdk/python/forge_sdk /tmp/forge_sdk
COPY sdk/python/forge_dq  /tmp/forge_dq
RUN pip install /tmp/forge_sdk /tmp/forge_dq
```

No runtime ADLS download. No `spark.submit.pyFiles`. Every executor has the SDK available immediately at pod start.
...
sdk/python/
├── forge_sdk/
│   ├── pyproject.toml          ← name="forge-sdk", version="1.0.0"
│   └── forge_sdk/
│       ├── __init__.py         ← ForgeJob, forge_session, path helpers
│       ├── config/platform.py  ← PlatformConfig
│       ├── job/base.py         ← ForgeJob abstract base class
│       ├── spark/session.py    ← forge_session() factory
│       └── storage/paths.py    ← bronze(), silver(), gold()
└── forge_dq/
    ├── pyproject.toml          ← name="forge-dq", version="1.0.0"
    └── forge_dq/
        ├── __init__.py         ← @track decorator
        ├── runner.py           ← DQRunner
        ├── rules/              ← NotNull, ValueRange, UniqueKey, ...
        └── output/             ← DeltaWriter, schemas
```

### When SDK Changes

SDK changes require a Spark image rebuild — not a `sync-jobs.sh` run:

```bash
# Rebuild Spark image and redeploy (--skip-infra, --skip-sync for speed)
bash infra/scripts/forge-up.sh --env dev --skip-infra --skip-sync
```

forge-up.sh Phase 5 builds all images including Spark. `--skip-build` skips this phase if the image is already current.

### Production Path

For prod, publish versioned wheels to Azure Artifacts and pin in the Dockerfile:

```bash
# Publish once per release
bash sdk/python/publish.sh

# Dockerfile becomes:
# RUN pip install forge-sdk==1.0.0 forge-dq==1.0.0 \
#     --index-url https://pkgs.dev.azure.com/L1R/.../_packaging/forge/pypi/simple/
```

This gives full semantic versioning, independent release cycles, and rollback by image tag.

---

## 5. Spark Configuration Hierarchy
...
Forge tracks the estimated cost of every compute job and attaches that estimate as a custom OpenLineage facet to the job's run event. This enables per-pipeline cost trending in the Developer Portal without requiring Azure Cost Management API calls for individual job attribution.
...
This facet is stored alongside all other run facets in the lineage store. The Developer Portal's Cost page reads `computeCost` facets from the portal lineage API, aggregates them by pipeline and time window, and displays cost trends.
...
Cross-cluster communication (Orchestration → Compute):
  ...
  Driver + Executor pods (spark-jobs namespace)
                                                                                   │
        │  OpenLineage events (internal)
        ▼
  Portal lineage API (consumed internally)
```
