...
4. [Spark Shared Library (forge_lib.zip)](#4-spark-shared-library-forge_libzip)
...
| Code/Lib | `abfss://code@<account>.dfs.core.windows.net/lib/` | Shared library (forge_lib.zip) |
...
## 4. Spark Shared Library (forge_lib.zip)

### Purpose

Every Spark job in Forge imports from two packages:
- `forge_sdk` — `ForgeJob` base class, `forge_session()`, ADLS path helpers, `PlatformConfig`
- `forge_dq` — `@track` decorator, DQ runner, rule types

These packages are **not baked into the Spark Docker image**. Instead they are distributed at runtime as a zip archive via `spark.submit.pyFiles`. This means:
- The image never needs rebuilding when the SDK changes
- Different jobs can reference different SDK versions by pointing to different zip paths
- The SDK is owned by the platform team and versioned independently of job code

### How It Works

```
sync-jobs.sh (when sdk/python/ changes):
        │
        │  cd sdk/python
        │  zip -r forge_lib.zip forge_sdk/ forge_dq/
        │  az storage blob upload → ADLS code/lib/forge_lib.zip
        ▼
SparkApplication YAML (generated by dag.ts for every job):
        │
        │  sparkConf:
        ...
        ▼
At job start:
  Driver:    downloads forge_lib.zip from ADLS → adds to sys.path
  Executors: forge_lib.zip distributed by Spark driver automatically
             → forge_sdk and forge_dq available on every executor
```

### ADLS Path

```
code/lib/forge_lib.zip
```
...
│   ├── config/platform.py ← PlatformConfig (FORGE_ENV, FORGE_STORAGE_ACCOUNT)
...
└── forge_dq/
    ...
    ├── runner.py          ← DQRunner
    ├── rules/             ← not_null, row_count, unique, ...
    └── output/            ← StoreReporter, LineageReporter
```

### Rebuild Trigger

`sync-jobs.sh` rebuilds `forge_lib.zip` only when `sdk/python/` has file changes since the last deployment. The check uses `git diff <last_commit>...HEAD -- sdk/python/`. On a pure manifest-only change, the existing zip is reused — no upload.

### Access Control

The compute cluster identity (`id-forge-compute-{env}`) has `Storage Blob Data Reader` on the `code/` container. Job pods access `forge_lib.zip` via the same workload identity as `mainApplicationFile` — no separate permissions needed.
